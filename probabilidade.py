...
st.markdown("<p style='text-align: center; color: #64748b; font-size: 1.1rem; margin-bottom: 2rem;'>Explorando a matemática do acaso com recursos gráficos interativos</p>", unsafe_allow_html=True)

# ============================================
# FUNÇÃO GERADORA DA ÁRVORE DINÂMICA
# ============================================
def build_tree_plotly(depth, tipo="moeda"):
    # Configuração dos ramos baseada na escolha
    if tipo == "moeda":
        branches = 2
        labels = ["C", "K"]
        ...
        branches = 6
        labels = ["1", "2", "3", "4", "5", "6"]
        colors = ["#f59e0b", "#3b82f6", "#8b5cf6", "#ec4899", "#14b8a6", "#f43f5e"] # Paleta diversa para o dado

    nodes = []
    edges = []

    # Função recursiva que divide o espaço vertical uniformemente
    def traverse(x, y_min, y_max, current_depth, path_label):
        y_mid = (y_min + y_max) / 2
        
        # O rótulo exibido no gráfico é só a última escolha
        display_label = "Início" if path_label == "" else str(path_label[-1])
        
        nodes.append({
            'x': x, 
            'y': y_mid, 
            'label': display_label, 
            'path': path_label if path_label else "Início",
        })

        if current_depth < depth:
            step = (y_max - y_min) / branches
            for i in range(branches):
                child_y_min = y_min + i * step
                child_y_max = y_min + (i + 1) * step
                child_y_mid = (child_y_min + child_y_max) / 2
                
                # Registra o galho (linha) conectando os nós
                edges.append({'x0': x, 'y0': y_mid, 'x1': x + 1, 'y1': child_y_mid})
                
                traverse(x + 1, child_y_min, child_y_max, current_depth + 1, path_label + labels[i])

    # Inicializa a criação da árvore (Altura total baseada no número máximo de folhas)
    traverse(0, 0, branches**depth, 0, "")
    
    fig = go.Figure()
    
    # Desenhar as linhas de conexão primeiro (ficam no fundo)
    for edge in edges:
        fig.add_shape(type="line", x0=edge['x0'], y0=edge['y0'], x1=edge['x1'], y1=edge['y1'], 
                      line=dict(color="#cbd5e1", width=1.5))
        
    # Extrair coordenadas e rótulos
    nx = [n['x'] for n in nodes]
    ny = [n['y'] for n in nodes]
    ntext = [n['label'] for n in nodes]
    nhover = [f"Sequência Obtida: {n['path']}" for n in nodes]
    
    # Lógica de Cores
    ncolors = []
    for n in nodes:
        if n['label'] == "Início":
            ncolors.append("#1e293b")
        elif tipo == "moeda":
            ncolors.append(colors[0] if n['label'] == "C" else colors[1])
        else:
            ncolors.append(colors[int(n['label'])-1])
            
    # Desenhar os Nós
    fig.add_trace(go.Scatter(
        x=nx, y=ny, mode="markers+text",
        text=ntext, 
        textposition="middle right" if branches**depth <= 16 else "top center",
        hovertext=nhover, hoverinfo="text",
        marker=dict(size=12 if branches**depth <= 16 else 8, color=ncolors, line=dict(width=1, color="white")),
        showlegend=False
    ))
    
    # Calcular altura dinâmica do gráfico para evitar que a árvore dos dados esmague
    num_leaves = branches**depth
    plot_height = max(400, num_leaves * 20) # Pelo menos 400px, ou 20px por folha final
    
    fig.update_layout(
        xaxis=dict(showgrid=False, zeroline=False, visible=False, range=[-0.2, depth + 0.8]),
        yaxis=dict(showgrid=False, zeroline=False, visible=False),
        plot_bgcolor='white', paper_bgcolor='white', height=plot_height, margin=dict(l=0, r=0, t=10, b=10)
    )
    return fig
...
with tab1:
    ...
    Quando eventos ocorrem em sequência e um não afeta o outro, utilizamos a <b>Regra do Produto</b> e multiplicamos as probabilidades. Cada ramificação representa uma etapa do experimento.
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("#### ⚙️ Configuração do Experimento")
        tipo_exp = st.radio("Selecione o Objeto:", ["Moeda (2 faces)", "Dado numérico (6 faces)"])
        
        is_moeda = (tipo_exp == "Moeda (2 faces)")
        base = 2 if is_moeda else 6
        
        # Limite didático: Dados crescem exponencialmente muito rápido
        max_lanc = 4 if is_moeda else 2
        qtd_lancamentos = st.slider("Quantidade de Lançamentos?", 1, max_lanc, 2)
        
        if not is_moeda and qtd_lancamentos == 2:
            st.info("💡 Perceba como a árvore com 2 dados já gera 36 ramificações! Três dados gerariam 216 ramificações e tornariam o gráfico ilegível.")
            
        total_resultados = base**qtd_lancamentos
        
        st.markdown(f"**Cálculo do Espaço Amostral:**")
        st.latex(rf"\Omega = {base}^{{{qtd_lancamentos}}} = {total_resultados} \text{{ resultados possíveis}}")
        
        st.markdown("**Probabilidade de cada resultado exato (folha da árvore):**")
        st.latex(rf"P = \left(\frac{{1}}{{{base}}}\right)^{{{qtd_lancamentos}}} = \frac{{1}}{{{total_resultados}}}")
        
    with col2:
        st.markdown("#### 🌳 Diagrama de Árvore Dinâmico")
        st.caption("Passe o mouse sobre os pontos para ver a sequência construída!")
        
        tipo_str = "moeda" if is_moeda else "dado"
        st.plotly_chart(build_tree_plotly(qtd_lancamentos, tipo=tipo_str), use_container_width=True)
